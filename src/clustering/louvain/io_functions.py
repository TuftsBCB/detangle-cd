...
def build_ig_graph_from_edgelist(input_file, is_directed=False):
    ...
    for line in graph_fp.readlines():
        ...
        if split_line[0] not in v_list:
            v_list.append(split_line[0])
        if split_line[1] not in v_list:
            v_list.append(split_line[1])

    #n = (max(max(node1, node2) for node1, node2 in edge_list))
