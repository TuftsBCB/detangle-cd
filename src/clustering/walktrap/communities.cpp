...
double Communities::merge_nearest_communities() {
  ...
  // check for communities that are too large first
  /*
  for (int lc = 0; lc < nb_communities; lc++) {
      // TODO: how to iterate through all active communities?
      // cout << "Size of community " << lc << " : " << communities[lc].size << endl;
      // cout << "Size of community " << lc << endl;
      if (communities[lc].size >= MAX_COMM_SIZE) {
          large_comms.push_back(lc);
          Neighbor* n1 = communities[lc].first_neighbor;
          // for all pairwise combinations including the large community,
          // remove them from neighbor-related data structures
          while (n1) {
              Neighbor* tmp = n1;
              if (n1->community1 == lc) n1 = n1->next_community1;
              else n1 = n1->next_community2;
              remove_neighbor(tmp);
              delete tmp;
          // TODO: worry about nb_active_communities?
          }
      }
  }
  */
  ...
  while(!N->exact || !N->size_checked) {
    ...
    else if (!N->size_checked) {
        while (!N->size_checked) {
            int merged_size = communities[N->community1].size + communities[N->community2].size;
            if (merged_size >= MAX_COMM_SIZE) {
                Neighbor* tmp = N;
                remove_neighbor(tmp);
                // delete tmp;
